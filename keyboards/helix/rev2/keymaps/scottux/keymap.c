...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [_LOWER] = LAYOUT( \
      ...
      _______, KC_F7,   KC_F8,   KC_F9,   KC_F10,  KC_F11,  KC_LPRN, KC_RPRN, KC_F12,  _______, RGB_TOG, RGB_HUI, RGB_SAI, RGB_VAI, \
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [_QWERTY] = LAYOUT( \
      ...
      ADJUST,  KC_ESC,  KC_LALT, EISU, KC_LGUI,   LOWER,   KC_SPC,  KC_SPC,  RAISE,   KANA,    KC_LEFT, KC_DOWN, KC_UP,   KC_RGHT \
      ),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  switch (keycode) {
    ...
    case KANA:
      if (record->event.pressed) {
        if(keymap_config.swap_lalt_lgui==false){
          register_code(KC_LANG3);
        }else{
          SEND_STRING(SS_LALT("`"));
        }
      } else {
        unregister_code(KC_LANG3);
      }
      return false;
      break;
    case RGBRST:
...
void render_status(struct CharacterMatrix *matrix) {
  ...
  matrix_write_P(matrix, PSTR("\nMODE : "));
    switch (layer_state) {
        case L_BASE:
           matrix_write_P(matrix, PSTR("terminal"));
           break;
        case L_RAISE:
           matrix_write_P(matrix, PSTR("netrun"));
           break;
        case L_LOWER:
           matrix_write_P(matrix, PSTR("crypto"));;;
           break;
        case L_ADJUST:
        case L_ADJUST_TRI:
           matrix_write_P(matrix, PSTR("META_MODE_DANGER"));
           break;
        default:
