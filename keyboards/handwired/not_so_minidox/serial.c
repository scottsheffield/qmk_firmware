...
// Used by the leader to synchronize timing with the follower.
static
void sync_recv(void) {
  serial_input();
...
// Used by the follower to send a synchronization signal to the leader.
static
void sync_send(void) {
  serial_output();
...
ISR(SERIAL_PIN_INTERRUPT) {
  ...
  for (int i = 0; i < SERIAL_LEADER_BUFFER_LENGTH; ++i) {
    serial_leader_buffer[i] = serial_read_byte();
    sync_send();
    checksum_computed += serial_leader_buffer[i];
  }
...
// Copies the serial_follower_buffer to the leader and sends the
// serial_leader_buffer to the follower.
//
// Returns:
// 0 => no error
...
int serial_update_buffers(void) {
  ...
  for (int i = 0; i < SERIAL_LEADER_BUFFER_LENGTH; ++i) {
    serial_write_byte(serial_leader_buffer[i]);
    sync_recv();
    checksum += serial_leader_buffer[i];
  }
  serial_write_byte(checksum);
  sync_recv();
