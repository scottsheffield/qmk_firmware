...
uint8_t i2c_leader_read(int ack) {
  ...
  switch(TW_STATUS) {
    ...
    case TW_SR_DATA_ACK:
      ...
      if(!follower_has_register_set) {
        ...
        // don't acknowledge the leader if this memory loctaion is out of bounds
    ...
    case TW_ST_SLA_ACK:
    case TW_ST_DATA_ACK:
      // leader has addressed this device as a follower transmitter and is
      // requesting data.
